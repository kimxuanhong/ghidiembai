...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Tạo nút gợi ý cài đặt nhỏ ở góc dưới phải
    createFloatingInstallButton();

    // Hiển thị nút nổi ngay lập tức để kiểm tra (chỉ dùng cho test)
    setTimeout(() => {
...
// Tạo nút gợi ý cài đặt nhỏ hình tròn ở góc dưới phải màn hình
function createFloatingInstallButton() {
    // Kiểm tra nếu nút đã tồn tại
    if (document.getElementById('floatingInstallButton')) return;
    
    // Kiểm tra nếu app đã được cài đặt
    if (isPWAInstalled()) return;
    
    const floatingButton = document.createElement('button');
    floatingButton.id = 'floatingInstallButton';
    floatingButton.className = 'floating-install-btn';
    ...
    floatingButton.title = 'Cài đặt ứng dụng';
    ...
    floatingButton.style.boxShadow = '0 4px 10px rgba(0,0,0,0.3)';
    floatingButton.style.cursor = 'pointer';
    floatingButton.style.zIndex = '9999';
    floatingButton.style.display = 'none'; // Bắt đầu với trạng thái ẩn
    ...
    floatingButton.addEventListener('mouseover', () => {
        ...
        floatingButton.style.boxShadow = '0 6px 14px rgba(0,0,0,0.4)';
    });
    ...
    // Kiểm tra nếu deferredPrompt đã tồn tại (sự kiện đã xảy ra trước khi tạo nút)
    if (deferredPrompt && !isIOS()) {
        showFloatingInstallButton();
    }
}

// Hiện nút cài đặt nổi
function showFloatingInstallButton() {
    const floatingButton = document.getElementById('floatingInstallButton');
    if (floatingButton) {
        floatingButton.style.display = 'flex';
    } else {
        // Nếu nút chưa tồn tại, tạo nút và hiển thị
        createFloatingInstallButton();
        const newButton = document.getElementById('floatingInstallButton');
        if (newButton) {
